...
export default function CanvasEditor({ params }:{ params:{ id:string } }){
  ...
  useEffect(()=>{ if(!id) return; const i = setInterval(async ()=>{ try{ const list = await databases.listDocuments(process.env.NEXT_PUBLIC_APPWRITE_DATABASE_ID||"", process.env.NEXT_PUBLIC_APPWRITE_PRESENCE_COLLECTION_ID||"", [`equal("canvasId","${id}")`]); setPresence(list.documents||[]); }catch(e){} },4000); return ()=>clearInterval(i); },[id]);
  ...
  const onDragEnd = (event:any)=>{ const { active, over } = event; if(!over) return; if(active.id !== over.id){ const oldIndex = canvas.blocks.findIndex((b:any)=> b.id === active.id); const newIndex = canvas.blocks.findIndex((b:any)=> b.id === over.id); const newBlocks = arrayMove(canvas.blocks, oldIndex, newIndex); setCanvas({...canvas, blocks: newBlocks}); persist({ blocks: newBlocks }); } };

  const aiSuggest = async ()=>{ try{ const res = await fetch('/api/ai/suggest', { method:'POST', body: JSON.stringify({ prompt: canvas?.title || 'Landing page' }) }); const json = await res.json(); if(Array.isArray(json.blocks)){ const gen = json.blocks.map((blk:any)=> ({ ...blk, id: uuidv4() })); const blocks = [...(canvas?.blocks||[]), ...gen]; setCanvas({...canvas, blocks}); await persist({ blocks }); } }catch(e){ console.error(e); } };

  const publish = async ()=>{ try{ const res = await fetch('/api/publish', { method:'POST', body: JSON.stringify({ id: canvas.$id }) }); const json = await res.json(); if(json.url) await persist({ published: true, publishedUrl: json.url }); alert(`Published: ${json.url || 'ok'}`); }catch(e){ console.error(e); } };
        ...
        <div className="col-span-12 md:col-span-3">
          <div className="glass p-4 rounded">
            ...
            <label className="block mb-2 text-sm">Title
              <input defaultValue={canvas.title} onBlur={(e)=> persist({ title: e.currentTarget.value })} className="w-full mt-1 p-2 rounded bg-slate-900 border"/>
            </label>
